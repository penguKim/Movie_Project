...
//						 										disabledTime.push(parseInt($("#play_start_time option").eq(j+2).val().split(":")[0]));
	...
	// 수정 버튼 클릭 시 호출 되는 CannotDuplicateModify() 함수 정의
	function CannotDuplicateModify(currentTrId, play_start_time, play_date) {
		...
		} else { // 영화 시작 시간이 현재 일시 이후인 경우 처리 시작
			// 클릭한 버튼이 속한 행을 선택합니다.
...
//						$("#" + String(result.play_id)).html(
				...
				}); // 이전 tr태그의 내용을 조회폼 형식으로 출력 ajax 끝
	        ...
	        $(row).find("#play_end_time_mod").html(endTime);
	     	...
	     	// 지점 불러오기 ajax 요청(1중) 시작
	        $.ajax({
				type: "GET",
				url: "getTheater",
				// --------------- 지점 불러오기 ajax success 내부 시작----------------
				success: function(result) {
					...
					$("#modifyTheater").blur(function() {
						...
						// 지점 선택에 따른 상영관 출력
						// 지점 불러오기 ajax 요청 내부에서 상영관 불러오는 또 다른 ajax 요청(2중) 시작
						$.ajax({
							type: "GET",
							url: "getRoom",
							data: {
	...
	//				$("#modifyTheater").toggle();
	    	...
	    	// 상영중인 영화 선택 ajax 요청(1중) 시작
			$.ajax({
				type: "GET",
				url: "nowPlaying",
				// --------------- 상영중인 영화 불러오기 ajax success 시작 ----------------
				success: function(result) {
					...
					$("#modifyMovie").blur(function() {
						...
						// 영화 선택에 따른 상영 가능한 날짜 불러오기
						// 상영중인 영화 불러오기 ajax 요청 내부에서 상영 가능한 날짜 불러오는 또 다른 ajax 요청(2중) 시작
						$.ajax({
							type: "GET",
							url: "getMovieInfo",
							data: {
							...
							// ------------------------------ 상영 가능한 날짜 불러오기 ajax success 시작 --------------------
							success: function(result) {
								if(movie_id != '') { // 영화 정보가 존재하는 경우		
									let release_date = new Date(result.movie_release_date + 86400000); // 밀리초단위를 날짜로 변환
									let f_release_date = release_date.toISOString().split('T')[0]; // 날짜의 형태를 0000-00-00으로 변환
									...
									$("#modifyDate").blur(function() {
										...
										// 날짜 선택에 따른 상영관의 기존 상영일정(시작 시간) 출력
										// 상영 가능한 날짜 불러오기 ajax 요청 내부에서 기존 상영일정(시작시간) 불러오는 또다른 ajax 요청(3중) 시작
										$.ajax({
											type: "GET",
											url: "getPlayTimeInfo",
											data: {
											...
											// ----------------------- 기존 상영일정(시작시간) 불러오기 ajax success 내부 시작 ------------------------
											success: function(result) {
								 				// console.log(JSON.stringify(result));
												// 각 배열에 가지고 있는 값: movie_id, play_date, play_end_time, play_id, play_start_time, play_turn, room_id
											...
											// ----------------------- 기존 상영일정(시작시간) 불러오기 ajax success 내부 끝 ------------------------

											error: function(xhr, textStatus, errorThrown) {
												alert("상영 날짜 출력 오류입니다.");
											}
										}); // 기존 상영일정(시작시간) 불러오는 또다른 ajax 요청(3중) 끝
										
									}); // 날짜 선택 후 blur 이벤트 처리 끝
									
									
								} // 영화 정보가 존재하는 경우 if문 끝
									
							},
							// ------------------------------ 상영 가능한 날짜 불러오기 ajax success 끝 --------------------

							error: function(xhr, textStatus, errorThrown) {
								alert("상영관 로딩 오류입니다.");
							}
						}); // 상영 가능한 날짜 불러오는 또 다른 ajax 요청(2중) 끝
					}); // 영화 선택 후 blur 이벤트 처리 끝
					...
					function selectable(isDisabled) {
						...
						// 상영중인 영화 불러오기 ajax 요청 내부에서 영화 정보 가져오는 ajax 요청(2중) 시작
						$.ajax({
							type: "GET",
							url: "getMovieInfo",
							data: {
						...
						}); // 상영중인 영화 불러오기 ajax 요청 내부에서 영화 정보 가져오는 ajax 요청(2중) 끝
						
					} // 러닝타임에 따라 영화를 비어있는 중간시간에 상영할수있는지 없는지를 판별하는 함수 끝	 
					
					
					// 영화 시작시간 선택 후 change 이벤트 처리
					// 영화 종료 시간 자동으로 계산
	...
	} // 수정 버튼 클릭 시 호출 되는 CannotDuplicateModify() 함수 정의 끝
